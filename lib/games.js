...
// Conditional cleanup for rows auto-created by date browsing: deletes only if
// the row is bare IN THE DB right now (all content columns null/empty) and has
// no screenshots — a client-side snapshot alone races concurrent saves/uploads
// (codex review 2026-08-26 round 2).
export async function deleteDayIfBare(id) {
    ...
    return supabase
        ...
        .eq('id', id)
        .is('temperature', null)
        .is('counter_value', null)
        .is('progress_note', null)
        .is('one_line', null)
        .eq('activities', '{}');
}
